...
//click en trasferir terceros
$("#btn-trasferirT").click(function() {
    if ($("#formTerceros").valid() === false) {
        return;
    }
    ...
    let accion = "transferirTercero";

    enviarAjaxTerceros(JSON.stringify(objDatos), accion);
})

function enviarAjaxTerceros(datos, accion) {
    $.ajax({
        async: true,
        url: "../ajax/ajax.php",
        type: "POST",
        ...
        beforeSend: function() {
            $("#btn-trasferirT").val("Espere por favor...");
            $("#btn-trasferirT").prop("disabled", true);
        },
        success: function(response) {    
            console.log(response);   
            let res = JSON.parse(response);
            ...
            $("#btn-trasferirT").prop("disabled", false);
            $("#btn-trasferirT").val("Transferir");
